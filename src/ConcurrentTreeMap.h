/**
 * forked from https://github.com/arunmoezhi/ConcurrentTreeMap
 */

#ifndef KRIEGSSPIEL_CONCURRENTTREEMAP_H
#define KRIEGSSPIEL_CONCURRENTTREEMAP_H
...
template<typename K, typename V>
ks::ConcurrentTreeMap<K, V>::ConcurrentTreeMap() {
    m_root = newLeafNode(std::numeric_limits<K>::min(), std::numeric_limits<V>::min());
    m_root->m_child[Direction::RIGHT] = newLeafNode(std::numeric_limits<K>::max(), std::numeric_limits<V>::max());
}

template<typename K, typename V>
ks::Node <K, V> *ks::ConcurrentTreeMap<K, V>::getAddress(Node<K, V> *p) {
    return (Node<K, V> *) ((uintptr_t) p & ~((uintptr_t) 3));
}

template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::isNull(Node<K, V> *p) {
    return ((uintptr_t) p & 2) != 0;
}

template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::isLocked(Node<K, V> *p) {
    return ((uintptr_t) p & 1) != 0;
}

template<typename K, typename V>
inline ks::Node <K, V> *ks::ConcurrentTreeMap<K, V>::setLock(Node<K, V> *p) {
    return (Node<K, V> *) ((uintptr_t) p | 1);
}

template<typename K, typename V>
inline ks::Node <K, V> *ks::ConcurrentTreeMap<K, V>::unsetLock(Node<K, V> *p) {
    return (Node<K, V> *) ((uintptr_t) p & ~((uintptr_t) 1));
}

template<typename K, typename V>
inline ks::Node <K, V> *ks::ConcurrentTreeMap<K, V>::setNull(Node<K, V> *p) {
    return (Node<K, V> *) ((uintptr_t) p | 2);
}

template<typename K, typename V>
inline bool
ks::ConcurrentTreeMap<K, V>::CAS(Node<K, V> *parent, int which, Node<K, V> *oldChild, Node<K, V> *newChild) {
...
template<typename K, typename V>
inline ks::Node <K, V> *ks::ConcurrentTreeMap<K, V>::newLeafNode(K key, V value) {
    return new Node<K, V>(key, value, setNull(nullptr), setNull(nullptr));
}

template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::lockEdge(Node<K, V> *parent, Node<K, V> *oldChild, int which, bool n) {
...
template<typename K, typename V>
inline void ks::ConcurrentTreeMap<K, V>::unlockEdge(Node<K, V> *parent, int which) {
    parent->m_child[which] = unsetLock(parent->m_child[which]);
}

template<typename K, typename V>
inline void
ks::ConcurrentTreeMap<K, V>::seek(K key, Node<K, V> **parent, Node<K, V> **node, Node<K, V> **injectionPoint) {
    Node<K, V> *prev[2];
    Node<K, V> *curr[2];
    ...
    BEGIN:
    {
        ...
        prev[cSeek] = m_root;
        curr[cSeek] = m_root->m_child[Direction::RIGHT];
        lastRNode[cSeek] = m_root;
        lastRKey[cSeek] = std::numeric_limits<K>::min();
        ...
        while (true) {
            ...
            if (isNull(temp)) {
                ...
                if (!isLocked(lastRNode[cSeek]->m_child[Direction::RIGHT])) {
...
template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::findSmallest(Node<K, V> *node, Node<K, V> *rChild, Node<K, V> **succNode,
                                                      Node<K, V> **succParent) {
    ...
    curr = rChild;
    while (true) {
        temp = curr->m_child[Direction::LEFT];
        n = isNull(temp);
        lChild = getAddress(temp);
        if (n) {
            break;
        }
        //traverse the next edge
    ...
    *succNode = curr;
    *succParent = prev;
    return prev == node ? true : false;
}

template<typename K, typename V>
V ks::ConcurrentTreeMap<K, V>::lookup(const K key) {
    Node<K, V> *parent;
    Node<K, V> *node;
...
template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::insert(K key, V value) {
    Node<K, V> *parent;
    Node<K, V> *node;
    Node<K, V> *injectionPoint;
    K nKey;
    ...
    while (true) {
        ...
        which = key < nKey ? Direction::LEFT : Direction::RIGHT;
...
template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::remove(K key) {
    Node<K, V> *parent;
    Node<K, V> *node;
    Node<K, V> *lChild;
    Node<K, V> *rChild;
    ...
    while (true) {
        ...
        pKey = parent->m_key;
        temp = node->m_child[Direction::LEFT];
        lChild = getAddress(temp);
        ln = isNull(temp);
        lLock = isLocked(temp);
        temp = node->m_child[Direction::RIGHT];
        rChild = getAddress(temp);
        rn = isNull(temp);
        rLock = isLocked(temp);
        pWhich = nKey < pKey ? Direction::LEFT : Direction::RIGHT;
        if (ln || rn) //simple delete
        {
            if (lockEdge(parent, node, pWhich, false)) {
                if (lockEdge(node, lChild, Direction::LEFT, ln)) {
                    if (lockEdge(node, rChild, Direction::RIGHT, rn)) {
                        if (nKey != node->m_key) {
                            unlockEdge(parent, pWhich);
                            unlockEdge(node, Direction::LEFT);
                            unlockEdge(node, Direction::RIGHT);
                            continue;
                        }
                        if (ln && rn) //00 case
                        {
        ...
        } else //complex delete
        {
            ...
            succNodeLChild = getAddress(succNode->m_child[Direction::LEFT]);
            temp = succNode->m_child[Direction::RIGHT];
            srn = isNull(temp);
            succNodeRChild = getAddress(temp);

            if (!lockEdge(node, rChild, Direction::RIGHT, false)) {
                continue;
            }
            while (true) {
                if (!isSplCase) //common case
                {
                    if (lockEdge(succParent, succNode, Direction::LEFT, false)) {
                        if (lockEdge(succNode, succNodeLChild, Direction::LEFT, true)) {
                            if (lockEdge(succNode, succNodeRChild, Direction::RIGHT, srn)) {
                                if (nKey == node->m_key) {
                                    node->m_key = succNode->m_key;
                                    if (srn) {
                                        succParent->m_child[Direction::LEFT] = setNull(succNode);
                                    } else {
                                        succParent->m_child[Direction::LEFT] = succNodeRChild;
                                    }
                                    unlockEdge(node, Direction::RIGHT);
                                    return true;
                                } else {
                                    unlockEdge(node, Direction::RIGHT);
                                    unlockEdge(succParent, Direction::LEFT);
                                    unlockEdge(succNode, Direction::LEFT);
                                    unlockEdge(succNode, Direction::RIGHT);
                                    break;
                                }
                            } else {
                                unlockEdge(succParent, Direction::LEFT);
                    ...
                    if (lockEdge(succNode, succNodeLChild, Direction::LEFT, true)) {
                        if (lockEdge(succNode, succNodeRChild, Direction::RIGHT, srn)) {
                            if (nKey == node->m_key) {
                                node->m_key = succNode->m_key;
                                if (srn) {
                                    succParent->m_child[Direction::RIGHT] = setNull(succNode);
                                } else {
                                    succParent->m_child[Direction::RIGHT] = succNodeRChild;
                                }
                                return true;
                            } else {
                                unlockEdge(node, Direction::RIGHT);
...
template<typename K, typename V>
inline unsigned long ks::ConcurrentTreeMap<K, V>::getSize(Node<K, V> *node, unsigned long sizeSoFar) {
    if (isNull(node)) {
        return sizeSoFar;
    }
    return 1 + getSize(node->m_child[Direction::LEFT], sizeSoFar) + getSize(node->m_child[Direction::RIGHT], sizeSoFar);
}

template<typename K, typename V>
unsigned long ks::ConcurrentTreeMap<K, V>::size() {
    return getSize(m_root, 0) - 2;
}

template<typename K, typename V>
inline void ks::ConcurrentTreeMap<K, V>::rangeQuery(std::list<Node<K, V> * > &out, Node<K, V> *node, K min, K max) {
    if (isNull(node)) {
        return;
    }
    if (node->m_key >= min) {
        rangeQuery(out, node->m_child[Direction::LEFT], min, max);
    }
    if (node->m_key >= min && node->m_key <= max) {
        out.push_back(node);
    }
    if (node->m_key <= max) {
        rangeQuery(out, node->m_child[Direction::RIGHT], min, max);
    }
}

template<typename K, typename V>
std::list<ks::Node<K, V> *> ks::ConcurrentTreeMap<K, V>::range(K min, K max) {
...
template<typename K, typename V>
inline bool ks::ConcurrentTreeMap<K, V>::isValidBST(Node<K, V> *node, K min, K max) {
    if (isNull(node)) {
        return true;
    }
    return node->m_key >= min && node->m_key <= max &&
           isValidBST(node->m_child[Direction::LEFT], min, node->m_key - 1) &&
           isValidBST(node->m_child[Direction::RIGHT], node->m_key + 1, max);
}

template<typename K, typename V>
bool ks::ConcurrentTreeMap<K, V>::isValidTree() {
    return (isValidBST(m_root, std::numeric_limits<K>::min(), std::numeric_limits<K>::max()));
}

#endif //KRIEGSSPIEL_CONCURRENTTREEMAP_H
